...
$\textit{Definition}$. An inferrable language, $L$, is a set of at least two sequences where each sequence satisfies a linear recurrence relation that forms a ring. Equivalently, given $A_i \in L$ for any $i \leq n$ where $A_i = a_i - \sum_{k \leq i}a_k = 0$ implies $a_i = \sum_{k < i}a_k$ then $a_i = \sum_{k < i}a_k$ thus a decision function is equivalent to $a_i = \sum_{j < i}A_j + \sum_{k < i}a_k$.
