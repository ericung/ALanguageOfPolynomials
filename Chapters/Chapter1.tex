...
Here, $A_n$ is the set of monomials in the polynomial. $A_{n-1}$ are the monomials of degree n-1 and less of the polynomial and the set ${\left\{  E^* | E \in \mathcal{E}_{n-1}, (E,1)=0 \right\}}$ contains $E_n$ or equivalently the top of a single state of a decider.
...
The focus of this article pertains to the question of whether or not there exists structures with certain properties that allow law of compositions to handle the above statement.
...
$Decider<cxyz^3n> \equiv cxyz^{3n} = Decider<cx^n> \times Decider<cy^n> \times Decider<cz^n>$ where c is some constant

$\\ $

$Decider<cx^{n+m+l}> \times  Decider<cy^m> \times  Decider<cz^l> = cx^n \times  y^m \times z^l$ where c is some constant

$\\ $

Given $f(x,y)=3xy^2$ and $g(x,y)=7x^7y^{-1}$

$\\ $

$f(x,y) \times g(x,y) = h(x,y) = 21x^8y$
