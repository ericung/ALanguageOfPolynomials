...
Here, $A_n$ is the set of monomials in the polynomial. $A_{n-1}$ are the monomials of degree n-1 and less of the polynomial and the set ${\left\{  E^* | E \in \mathcal{E}_{n-1}, (E,1)=0 \right\}}$ is equivalent to $Decider<cx^n>$ or equivalently the top of a single state of a decider. $E$ is the decider denoted Decider<$m_{n-1}(x)$>. $(E,1)$ is the constant such that it has 0 as the value. This theory has the opinion that it tries to describe finite recursion and nested for loops in a visual manner and, in so doing, this chain is chosen to be the best definition.
...
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.5]{0111MultipleDivisions.jpg}
...
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.5]{0106Multivariables.jpg}
...
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.5]{0112Equivalence.jpg}
...
1. $x = x$ from $\textbf{theorem of equivalence}$

2. $x \neq x$ from $\textbf{theorem of reversing}$

$\\ $

$\textbf{Example}$: Given some $d_1$,$d_2$,$d_3$,...,$\infty$ in decision functions in $Decider<m(x)>$
