...
$\textbf{Theorem}$. A monomial decider represented as, $A_n$, can be mapped into a set consisting of the base layer, $E_0$.
...
$\textit{Proof}$. For every $E_k$ in $E_n$, $E_k$ is a state that connects to $E_{k-1}$ where $1<k<n$.

For every $E_k$ to $E_{k-1}$, every semigroup of $q_{finish} \in E_k$ connects to the $q_{start}$ at each layer $E_{k-1}$.

Given $q_{finish}\in E_0$ in one group connecting to another group on $q_{start}\in E_k$, remove the connection and connect it to $q_{start}\in E_{k-1}$ where $1\leq k\leq n$ and n is a degree of the monomial.

This implies that $\cup E_0$ hence it is a cyclic automata consisting of semigroups of $A_0$ that decides y on the monomial m(x).
