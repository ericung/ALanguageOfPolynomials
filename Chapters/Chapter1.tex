...
$Decider<x_{0,j,0}^n>\subset Decider<x_{i,j,k}^n>\subset Decider<c x^{n}>$
...
Furthermore, it can be noted that there exists a start state,$q_{start}$ and a finish state, $q_{finish}$, for each semigroup in the decider.
...
Unraveling a monomial decider can be reduced down to the layer $\mathcal{E}_1$ and $\mathcal{E}_0$. In the following proof of the theorem below, it can be noted that $q_{finish}$ isn't the same as $q\in q_{accept}$.
...
$\textit{Proof}$. Given $m_1 = c_1 x^n$ and $m_2 = c_2 x^n$,

There exists deciders on $m_1$ and $m_2$ denoted $Decider<c_1 x^n>$ and $Decider<c_2 x^n>$.

Use the corollary of addition to form a decider on the aggregate of $m_1$ and $m_2$ called $m_3$ such that 
...
By definition, this is equivalent to $Decider<c x^n>$.

For the reverse, take $c x^n$.

Use the splitting corollary to get deciders on $c_i x^n$ and $c_{i+1} x^n$.

Hence $Decider<c x^n>\implies Decider<c_i x^n> + Decider<c_{i+1} x^n>$

$\implies Decider<m_1> + Decider<m_2$
