...
$\textbf{Theorem}$. A monomial decider can be, $A_n$, can be mapped into a set consisting of the base layer, $E_0$.
...
Given $q_{finish}$ of $E_0$ in one group connecting to another group on $q_{start}\in E_k$, remove the connection and connect it to $q_start\in E_{k-1}$ where $1\leq k\leq n$ and n is a degree of the monomial.

This gives a union of $E_0$ hence it is a cyclic automata consisting of semigroups of $E_0$ that decides y on the monomial m(x).
