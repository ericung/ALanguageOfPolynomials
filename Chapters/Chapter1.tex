...
Each state in the decider has monomials the same property of being able to describe the lesser monomial which it contains and the following example is how it is denoted.

$\\ $

Contains $Decider<3 x_{3}^3> \equiv 3x^3 = y$

Contains $Decider<3 x_{2}^3>\cup \cdots\cup Decider<3 x_{2}^3> \equiv 3x^2 = y$

Contains $Decider<3 x_{1}^3>\cup \cdots\cup Decider<3 x_{1}^3> \equiv 3x^1 = y$

Contains $Decider<3 x_0^0>\cup \cdots\cup Decider<3 x_0^0> \equiv 3x^0 = y$

$\\ $

Hence it can be generalized to:

$Decider<c x^{n}>$ contains the set of deciders
...
Contains $Decider<c x_{1}^{n}>\cup \cdots\cup Decider<c x_{1}^{1}>$ = $\mathcal{E}_{1}$

Contains $Decider<c>\cup \cdots\cup Decider<c>$ = $\mathcal{E}_{0}$
...
$\textit{Proof}$. For every $E_k$ in $A_n$, $E_k$ is a state that connects to $E_{k-1}$ where $2<k<n$.
...
$\textit{Proof}$. To show that a space is locally compact at a state, $q$, suppose that there is a subspace C of the topology of $A_n$ contains a neighborhood of $q$. This subspace is the topology of $E_1$. All subspace of the topology of $E_k$ such that $1<k\leq n$ contains $E_1$. Every state, $q$, of the topology of $E_k$ contains the subspace of the topology of $E_1$ hence $E_k$ is locally compact. Since $E_k$ is locally compact at every state, the topology of $A_n$ is locally compact.

$\\ $

$\textbf{Theorem}$. A monomial decider is a locally compact Hausdorff space.
...
$m_2=Decider<3 x_^2>=3x^2$

$m_1=Decider<4 x^1>=4 x$
